        ...
        {options.map((item) => (
          <Grid item xs={6} md={3} lg={2} key={item.value}>
            <CardItemsOption
              ...
              imgSrc={item.imgSrc}
              text={item.name}
              selectHandler={() => handleChange(item.value)}
            />
          </Grid>
        ))}
        {allowNotSure &&
          options.findIndex((item) => item.value === "not-sure") === -1 && (
            <Grid item xs={6} md={3} lg={2} key="not-sure">
              <CardItemsOption
                active={"not-sure" === value}
                text={"Not Sure"}
                selectHandler={() => handleChange("not-sure")}
              />
            </Grid>
          )}
      </Grid>
