  ...
  return (
    <Grid item md={12} onClick={() => handleOpen(val)} className="w-full">
      <Box className="space-y-4 bg-slate-50 p-4 rounded-lg">
        <div className="md:flex md:items-center md:gap-4 space-y-4">
          <img
            src={
              val?.product?.img?.[0] ?? val?.products?.[0]?.product?.img?.[0]
            }
            alt="img_prod"
            className="w-full aspect-video object-cover rounded-md md:w-44 md:h-44"
          />
          <div className="space-y-4">
            ...
            {val?.status && (
              <Chip
                label={
                  invoiceStatus.find((v: any) => v.status === val?.status)
                    ?.label
                }
