...
function Cart() {
  ...
  const handleDelete = async (product: any) => {
    ...
    const updatedCart = cartList?.filter((val: any) => val.productId !== product.productId);
  ...
  const selectAllProducts = () => {
    if (selectedProducts?.length === cartList?.filter((product) => product.qty > 0 && product.delete === false).length) {
      setSelectedProducts([]);
    } else {
      setSelectedProducts(cartList?.filter((product) => product.qty > 0 && product.delete === false).map((product) => product.id));
    }
  };
  ...
  const getTotalPrice = () => {
    return selectedProducts.reduce((totalPrice, productId) => {
      const product = cartList?.find((p) => p.id === productId);
      return product ? totalPrice + parseInt(product.unitPrice) * product.qty : totalPrice;
    }, 0);
  };
        ...
        <Box className="flex flex-col content-center">
          <Box className="h-min-screen flex flex-col justify-center bg-white">
            <Container maxWidth="xl" className="px-4 md:px-16 pt-36 pb-28 md:pt-48 md:pb-48">
              <Typography fontSize={26} fontWeight={700} color="text.primary">{t('cart.title')}</Typography>
              <Box className="flex items-center">
                <Checkbox onClick={selectAllProducts} />
                <Typography fontSize={17} fontWeight={400} color="text.primary">{t('cart.selectAll')}</Typography>
              </Box>
                ...
                <>
                  {val?.delete ? null : <>
                    <Box className="mt-5 flex justify-between">
                      ...
                      <Box>
                        <Button onClick={() => handleDelete(val)} className='ml-7 text-red-500'>hapus</Button>
                        <Box className="flex items-center">
                          <button disabled={val.qty === 1} onClick={() => adjustProductQuantity(val.id, val.qty - 1)} className='w-7 h-7 bg-transparent outline-none border-slate-800 rounded-full cursor-pointer'>-</button>
                          <Typography fontSize={17} marginLeft="15px" marginRight="15px" fontWeight={600} color="text.primary">{val.qty}</Typography>
                          <button disabled={val.qty === val.product?.stock} onClick={() => adjustProductQuantity(val.id, val.qty + 1)} className='w-7 h-7 bg-transparent outline-none border-slate-800 rounded-full cursor-pointer'>+</button>
                        </Box>
                      </Box>
                    </Box>
                    <hr className="h-px my-2 bg-[#E2E2E2] border-0 dark:bg-[#E2E2E2]" /></>}
              ...
              <Box className="mt-7 flex justify-between items-center">
                <Typography fontSize={17} marginLeft="15px" marginRight="15px" fontWeight={500} color="text.primary">Total Harga:  <span className='font-bold'>{rupiah(getTotalPrice())}</span></Typography>
                <button disabled={selectedProducts?.length === 0} onClick={() => buySelectedProducts()} className='w-16 h-9 bg-[#713F97] border-none rounded-md text-slate-50 cursor-pointer'>Beli</button>
              </Box>
            </Container>
          </Box>
        </Box>
