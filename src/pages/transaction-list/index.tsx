...
const TransactionListIndex = () => {
    ...
    return (
        <div className="py-40 flex-grow flex-fill">
            <Container>
                <Typography variant="h5" fontWeight={600} marginBottom='10px'>Order List</Typography>
                <Box className='mb-8 flex items-center'>
                    <Typography marginRight='30px'>Product Category</Typography>
                    <div onClick={() => handleChangeTabs('cp')} className={`p-3 mr-5 rounded-md cursor-pointer text-white ${isTab === 'cp' ? 'bg-[#713F97]' : 'bg-[#bb86fc]'}`}>Custom Packaging <span className="bg-[#cfd6c8] rounded-full p-1">5</span></div>
                    <div onClick={() => handleChangeTabs('dp')} className={`p-3 mr-5 rounded-md cursor-pointer text-white ${isTab === 'dp' ? 'bg-[#713F97]' : 'bg-[#bb86fc]'}`}>Digital Packaging <span className="bg-[#cfd6c8] rounded-full p-1">2</span></div>
                    <div onClick={() => handleChangeTabs('rtb')} className={`p-3 mr-5 rounded-md cursor-pointer text-white ${isTab === 'rtb' ? 'bg-[#713F97]' : 'bg-[#bb86fc]'}`}>Ready To Buy <span className="bg-[#cfd6c8] rounded-full p-1">7</span></div>
                </Box>
                <Box>
